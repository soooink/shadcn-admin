...
import { PluginBatchOperations } from './components/plugin-batch-operations'
import { useToast, PluginToast } from './components/plugin-toast'
import { PluginCloudSync } from './components/plugin-cloud-sync'
import { PluginExportDialog } from './components/plugin-export-dialog'
...
export default function Plugins() {
  ...
  const handleGroupSelect = (groupId: string | null) => {
    ...
    setSelectedPluginIds([]) // 清除选中的插件
  }
  
  // 批量操作相关函数
  const handleToggleSelectAll = () => {
    if (selectedPluginIds.length === filteredAndSortedPlugins.length) {
      // 如果全部选中，则取消全选
      setSelectedPluginIds([])
    } else {
      // 否则全选
      setSelectedPluginIds(filteredAndSortedPlugins.map(plugin => plugin.id))
    }
  }
  ...
  const handleToggleSelectPlugin = (pluginId: string) => {
  ...
  const handleAddSelectedToGroup = async (groupId: string) => {
    ...
    try {
      ...
      if (savedGroups) {
        const groups = JSON.parse(savedGroups)
        const groupIndex = groups.findIndex((g: {id: string}) => g.id === groupId)
        ...
        if (groupIndex !== -1) {
          ...
          selectedPluginIds.forEach(pluginId => {
            if (!updatedGroup.pluginIds.includes(pluginId)) {
              updatedGroup.pluginIds.push(pluginId)
            }
          })
          
          // 更新分组列表
          groups[groupIndex] = updatedGroup
          localStorage.setItem('plugin-groups', JSON.stringify(groups))
  ...
  const handleRemoveSelectedFromGroup = async () => {
    ...
    try {
      ...
      if (savedGroups) {
        const groups = JSON.parse(savedGroups)
        const groupIndex = groups.findIndex((g: {id: string}) => g.id === selectedGroupId)
        ...
        if (groupIndex !== -1) {
          ...
          // 从分组移除选中的插件
          updatedGroup.pluginIds = updatedGroup.pluginIds.filter(
            (id: string) => !selectedPluginIds.includes(id)
          )
  ...
  // 应用推荐排序
  const handleApplyRecommendedOrder = (sortedPluginIds: string[]) => {
    // 根据推荐顺序重新排序插件
    // 注意：我们只需要保存插件ID的顺序，不需要实际重排插件数组
    
    // 保存自定义排序
    setCustomOrder(sortedPluginIds)
    localStorage.setItem('plugin-custom-order', JSON.stringify(sortedPluginIds))
    
    // 切换到自定义排序模式
    setSortBy('custom')
    
    // 显示成功提示
    showSuccess(t('orderApplied'))
  }
  ...
  return (
    <>
      {/* 提示容器 */}
      <PluginToast toastMessages={toasts} removeToast={removeToast} />
      
      {/* ===== 云同步对话框 ===== */}
      <PluginCloudSync 
        isOpen={isCloudSyncOpen}
        onClose={() => setIsCloudSyncOpen(false)}
        onSyncSuccess={() => showSuccess(t('syncSuccess'))}
      ...
      <PluginExportDialog 
        ...
        onExportSuccess={() => showSuccess(t('exportSuccess'))}
        onExportError={(error) => showError(error)}
        onImportSuccess={() => {
          showSuccess(t('importSuccess'))
          refreshPlugins()
        }}
        onImportError={(error) => showError(error)}
      />
      ...
      <PluginUpdateDialog 
        ...
        plugins={plugins}
        selectedPluginIds={selectedPluginIds}
        onUpdateSuccess={(updatedIds) => {
          showSuccess(t('updateSuccess'))
          refreshPlugins()
        }}
        onUpdateError={(error) => showError(error)}
      />
                ...
                <ul className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {currentPagePlugins.map(plugin => (
                    <li 
                      ...
                      className={`transition-opacity ${draggedPlugin === plugin.id ? 'opacity-50' : 'opacity-100'} ${selectedPluginIds.includes(plugin.id) ? 'border-2 border-primary rounded-lg' : ''}`}
                      onClick={(e) => {
                        // 如果按住 Ctrl 键或 Command 键点击，则切换选中状态
                        if (e.ctrlKey || e.metaKey) {
                          e.preventDefault()
                          handleToggleSelectPlugin(plugin.id)
                        }
                      }}
                    >
                      <PluginCard
